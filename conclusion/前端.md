...
1. 主要区别计算属性 会被 vue 中的\_\_ob\_\_记录,可以缓存值,每当值进行改变的时候触发更新,method 不能
...
#### 在 2.6 中 VUE 主要使用的是微任务触发,先使用 Promise 而在 2.5 中使用宏任务和微任务结合使用

---

### Vue 内部 $nextTick 方法

> 原理其实很简单 用 callbacks 模拟消息队列 将待执行的函数放入 callbacks 里 然后延迟执行 如果没有 cb 方法 那么返回 promise 对象
...
Vue.prototype._render = function() {
  ...
  var vnode;
  vnode = render.call(
    vm._renderProxy, // 走proxy 代理  将this代理到 Vue上 寻找上面的方法
    vm.$createElement
  );
  return vnode;
};
// 将 render 即上面的render 渲染函数 通过代理方式 将其中的_c _v等渲染函数直接执行
// 最后返回 Vnode函数
...
- 最终渲染完的 v-show = true 的 VNode 会有在 data 中 有一个指令 show 其 value 为 true
- 而 v-show = false 的 value 为 false

#### v-if

```javascript
// 渲染语法很简单
<div v-if="false">v-if 源码展示</div>
<div v-if="true">v-if 源码展示</div>

// 当为true 创建元素 否则创建 空元素 _e创建空元素
false ? _c("div", [_v("v-if 源码展示")]) : _e(),
true ? _c("div", [_v("v-if 源码展示")]) : _e(),
